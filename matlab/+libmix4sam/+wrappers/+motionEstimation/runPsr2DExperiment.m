...
%RUNPSR2DEXPERIMENT Do a simple 2D point set registration using SA.
%
...
addParameter(p,'gmmImplementation', 'Sum', validFct);
% NOT IMPLEMENTED
...
% NOT IMPLEMENTED
p = rmfield(p,{'doOutlierModeling','doNumericalHessian','useScaling'}); 
...
for iLmSet = size(experiment,2):-1:1
    ...
    parfor i=1:size(experiment,1)
        ...
        estimation(i,iLmSet) = libmix4sam.wrappers.motionEstimation.optimizePsr2DProblem(...
            previous.Coordinates, current.Coordinates,...
            previous.Covariance, current.Covariance,...
            'init', init, 'variant', p.gmmImplementation);
